...
def start_schedule():
    try:
        ...
        if not interval_str.isdigit():
            messagebox.showwarning("警告", "请输入有效的正整数作为间隔")
            return
        interval = int(interval_str)
        if interval <= 0:
            messagebox.showwarning("警告", "间隔必须大于0")
            return
        ...
        if repeat:
            log(f"已启动任务: {task_name}，每{interval}秒执行一次")
        else:
            log(f"已启动任务: {task_name}，仅执行一次")
...
start_btn = tk.Button(root, text="启动任务", command=start_schedule)
